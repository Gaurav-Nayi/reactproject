...
const View = () => {
  ...
  <tbody>
    {users.length == 0 && <tr><td colSpan={6}>No User Found</td></tr>}
    {users.map((user,i)=>
    <tr key={i}>
      ...
      <td class="border border-slate-300 "><button type="button" className="round-md bg-red-600 px-3
       py-1.5 text-sm font-semibold leading-6 text-white shadow-sm" onClick={()=>dispatch(REMOVE_USER(user.id))}>Remove</button>
      </td>
    </tr>)}
  </tbody>
</table>
    <button type="button" className="round-md bg-red-600 px-3 py-1.5 text-sm font-semibold leading-6
     text-white shadow-sm" onClick={()=>dispatch(REMOVE_ALL_USERS())}>Remove All Users</button>
